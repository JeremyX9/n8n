...
export class ExecutionPersistence {
	...
	/**
	 * Delete Azure-stored execution data. If the Azure store is unavailable, e.g. external
	 * storage was unconfigured after Azure-stored executions were created, we skip data
	 * deletion rather than block entity deletion.
	 */
	...
	private getStoreFor(location: ExecutionDataStorageLocation): ExecutionDataStore {
		switch (location) {
			...
			case 'az':
				if (!this.azStore) {
					throw new UnexpectedError(
						'Execution data is stored on Azure Blob but the Azure store is not initialized. Check that Azure is configured.',
					);
				}
				return this.azStore;
		}
